...
class CanvasComponent extends React.Component {
    ...
    drawArea(ctx, i, width, height) {
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.strokeStyle = "#f9ffd0";
        ctx.fillStyle = "#f9ffd0";
    ...
    drawGraph(r, i, canvas) {
        ...
        let height = canvas.height;
        let width = canvas.width;
        ctx.strokeStyle = "#FCF751";
        ctx.fillStyle = "#FCF751";
        ctx.beginPath();
        ctx.moveTo(width / 2, height / 2);
        ctx.lineTo(width / 2 + r / 2 * i, height / 2);
        ctx.lineTo(width / 2, height / 2 - r * i);
        ctx.lineTo(width / 2, height / 2 - r / 2 * i);
        ctx.arc(width / 2, height / 2, r / 2 * i, Math.PI / 2 * 3, Math.PI * 3, true);
        ctx.moveTo(width / 2 - r / 2 * i, height / 2);
